...
async def send_payment(session, stats, semaphore):
    async with semaphore:
        ...
        customer_ref = f"SIM-{int(time.time()*1000)}"
        
        is_inquiry = random.random() < 0.3
        
        start_time = time.perf_counter()
        try:
            if is_inquiry:
                inquiry_url = f"http://localhost:8080/api/v1/debts?service_id={service_id}&customer_reference={customer_ref}"
                async with session.get(inquiry_url, timeout=5) as resp:
